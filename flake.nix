      ...
      flake =
        let
          ...
          hostnames = nixpkgs.lib.filter (
            name: hostEntries.${name} == "directory" && builtins.pathExists (./hosts + "/${name}/meta.nix")
          ...
          mkHost =
            ...
            nixpkgs.lib.nixosSystem {
              inherit system;
              specialArgs = {
                inherit inputs; # <- Благодаря этому inputs доступен во всех модулях
                host = hostname;
